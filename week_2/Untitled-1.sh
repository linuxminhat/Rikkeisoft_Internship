#!/bin/bash

# Hàm thực hiện Google Dorking
google_dork() {
    query="$1"
    encoded_query=$(echo "$query" | sed 's/ /%20/g')
    url="https://www.google.com/search?q=$encoded_query"
    
    # Sử dụng curl để lấy kết quả tìm kiếm
    results=$(curl -s -A "Mozilla/5.0" "$url")
    
    # Trích xuất các liên kết từ kết quả
    echo "$results" | grep -Eo "(http|https)://[a-zA-Z0-9./?=_-]*" | sort -u
}

# Query cac muc tieu tim kiem bang Google Dorking 
queries=(
    "site:leetcode.com inurl:array hard"
)

# Thuc hien truy van 
for query in "${queries[@]}"
do
    echo "Executing: $query"
    google_dork "$query" > "results_$(echo "$query" | sed 's/[^a-zA-Z0-9]/_/g').txt"
    echo "Saved Results."
    echo
done

echo "All queries completed."
             